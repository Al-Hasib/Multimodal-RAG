...
class MultiModalRetriever:
    ...
    def retrieve(self, query: str, k: int | None = None, metadata_filter: dict | None = None) -> RetrievalResult:
        top_k = k or self.k
        logger.info(f"Retrieving for query with k={top_k}: {query}")

        # Self-query: parse natural language into structured query + filter
        structured = self.self_query.parse_query(query)
        base_query = structured.query or query
        qdrant_filter = metadata_filter or self.self_query.build_qdrant_filter(structured.filter_expr)

        if qdrant_filter:
            logger.info(f"Applying Qdrant filter: {qdrant_filter}")

        queries = self.query_transformer.transform(base_query)
        all_docs: list = []

        for q in queries:
            if self.hybrid:
                docs = self._hybrid_search(q, top_k, filter_condition=qdrant_filter)
            else:
    ...
    def _hybrid_search(self, query: str, k: int, filter_condition: dict | None = None) -> list:
        ...
        if filter_condition:
            search_kwargs["query_filter"] = QdrantFilter(**filter_condition)
